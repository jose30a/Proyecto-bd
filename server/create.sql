...
CREATE TABLE usuario (
    ...
    fk_cod_rol INTEGER REFERENCES rol(cod)
);
...
CREATE TABLE lugar (
    ...
    fk_cod_lug_padre INTEGER REFERENCES lugar(cod_lug)
);
...
-- =============================================
-- Consolidated into Dashboard section (see line 1642)
-- =============================================
-- Placeholder / Basic Analytics
-- =============================================
-- =============================================
...
$$ LANGUAGE plpgsql;
-- =============================================
-- 10.1 REAL REPORT IMPLEMENTATIONS
-- =============================================
...
$$ LANGUAGE plpgsql;
-- =============================================
-- 12. PROMOTION SERVICE ASSIGNMENT
-- =============================================
...
FOR v_service IN
SELECT DISTINCT ON (fk_servicio, inicio_ser, fin_ser) fk_servicio,
    costo_ser,
    inicio_ser,
    fin_ser,
    millaje_ser
...
WHERE fk_paquete = p_booking_id
    ...
    AND inicio_ser = v_service.inicio_ser
    AND fin_ser = v_service.fin_ser
        ...
        (
            SELECT STRING_AGG(
                    p_child.nombre_paq || ' ($' || COALESCE(
                        (
                            SELECT SUM(s_c.costo_ser) + COALESCE(
                                    (
                                        SELECT SUM(costo_reserva_hot)
                                        FROM hot_paq
                                        WHERE fk_paquete = p_child.cod
                                    ),
                                    0
                                ) + COALESCE(
                                    (
