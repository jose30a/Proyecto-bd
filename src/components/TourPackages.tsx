...
export function TourPackages() {
  ...
  const [newTagData, setNewTagData] = useState<Partial<DbTag>>({
    nombre_tag: '',
    condicion1_tag: '',
    condicional_tag: '',
    condicion2_tag: '',
    restriccion_tag: false
  });
  ...
  const availableTagSuggestions = tagSuggestions.filter(
    (tag: string) => !allTags.some((t: DbTag) => t.nombre_tag === tag && selectedTagIds.includes(t.cod)) && (tag || '').toLowerCase().includes((tagInput || '').toLowerCase())
  );
    ...
    (async () => {
      try {
        ...
        const mapped = await Promise.all((rows || []).map(async (r: any) => {
          ...
          // fetch tags
          const pTags = await getPackageTags(pkgId).catch(() => []);
  ...
  const filteredPackages = packages.filter((pkg: TourPackage) => {
  ...
  const handleEdit = async (pkg: TourPackage) => {
    ...
    setSelectedTagIds(pkg.tags.map(t => t.cod));
    setIsDetailView(true);
  ...
  const loadPackageDetails = async (pkgId: number) => {
    try {
      ...
      const svc: Service[] = (details || []).map((d: any, idx: number) => ({
        id: d.item_id || idx,
        type: d.item_type ? (d.item_type.charAt(0).toUpperCase() + d.item_type.slice(1)) as Service['type'] : 'Flight', // Default to 'Flight' or handle unknown types
        name: d.item_name,
        startDate: d.inicio || '',
        endDate: d.fin || '',
        cost: d.costo ? Number(d.costo) : 0,
      ...
      setServices(svc);
      setFormData((prev: any) => ({ ...prev, totalCost, capacity, duration }));
    } catch (err: any) {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      setPackages(packages.filter((pkg: TourPackage) => pkg.id !== id));
    } catch (err: any) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingPackage) {
        ...
        await upsertPackage({ id: 0, name: formData.name, description: formData.description, status: formData.status, millaje: 0, costo_millas: 0, huella: 0 });
        const allPkg = await getAllPackages();
        pkgId = Math.max(...allPkg.map((p: any) => p.p_cod ?? p.id), 0);
      }

      // Sync tags
      await removeAllTagsFromPackage(pkgId);
      for (const tagId of selectedTagIds) {
        await assignTagToPackage(tagId, pkgId);
      }

      const refreshedTags = await getPackageTags(pkgId);
      if (editingPackage) {
        const updated = packages.map((pkg: TourPackage) => pkg.id === editingPackage.id ? { ...pkg, ...formData, services, tags: refreshedTags } : pkg);
        setPackages(updated);
      } else {
        const newPackage: TourPackage = { id: pkgId, ...formData, services, tags: refreshedTags };
  ...
  const handleAddService = async () => {
    ...
    try {
      if (editingPackage) {
        let dbType: 'hotel' | 'restaurant' | 'transport' = 'transport';
  ...
  const handleRemoveService = async (id: number, type: Service['type']) => {
    if (confirm('Remove this service?')) {
      if (editingPackage) {
        try {
          ...
          await removeItemFromPackage(editingPackage.id, id, dbType);
          await loadPackageDetails(editingPackage.id);
        } catch (err) {
          console.error(err);
  ...
  const handleTagInputKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === 'Enter' && tagInput.trim()) {
      e.preventDefault();
      // find tag by name in allTags
      const matched = allTags.find((t: DbTag) => t.nombre_tag.toLowerCase() === tagInput.toLowerCase());
      if (matched) {
        handleAddSelectedTag(matched.cod);
      }
    }
  };

  const handleAddSelectedTag = (tagId: number) => {
  ...
  const handleRemoveSelectedTag = (tagId: number) => {
    setSelectedTagIds(selectedTagIds.filter((id: number) => id !== tagId));
  };

  const handleCreateTag = async () => {
    if (!newTagData.nombre_tag) {
      alert('Tag name is required');
      return;
    }
    try {
      await upsertTag(newTagData);
      await loadAllTags();
      setNewTagData({
        nombre_tag: '',
        condicion1_tag: '',
  ...
  return (
    <div>
          ...
          <div className="bg-[var(--color-card)] border border-[var(--color-border)] rounded-lg p-6 mb-6">
            <div className="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-2.5 bg-[var(--color-input-bg)] hover:bg-[var(--color-border)] text-[var(--color-text-primary)] rounded-md transition-colors border border-[var(--color-border)] whitespace-nowrap"
              >
                <TagIcon className="w-5 h-5 text-[var(--color-primary-blue)]" />
            ...
            <div className="lg:col-span-1">
              <div className="bg-[var(--color-card)] border border-[var(--color-border)] rounded-lg sticky top-6">
                ...
                <div className="p-6 space-y-4">
                      ...
                      <div className="absolute z-10 w-full mt-1 bg-[var(--color-card)] border border-[var(--color-border)] rounded-md shadow-lg max-h-48 overflow-y-auto">
                        {availableTagSuggestions.map((tag: string) => (
                          <button
                            key={tag}
                            onClick={() => {
                              const t = allTags.find((item: DbTag) => item.nombre_tag === tag);
                              if (t) handleAddSelectedTag(t.cod);
                            }}
                    ...
                    {selectedTagIds.length > 0 ? (
                      <div className="flex flex-wrap gap-2">
                        {selectedTagIds.map((id: number) => {
                          const t = allTags.find((tag: DbTag) => tag.cod === id);
                          if (!t) return null;
                          return (
                            <span
                              key={t.cod}
                              className={`inline - flex items - center gap - 1.5 px - 3 py - 1.5 rounded - md text - sm ${t.restriccion_tag ? 'bg-red-500' : 'bg-[var(--color-primary-blue)]'} text - white`}
                            >
                              {t.nombre_tag}
                              <button
                                onClick={() => handleRemoveSelectedTag(t.cod)}
                  ...
                  {/* Tag Suggestions Display (repurposed for all available tags) */}
                  <div>
                    <p className="text-sm text-[var(--color-text-secondary)] mb-3">Available Tags:</p>
                    <div className="flex flex-wrap gap-2 max-h-40 overflow-y-auto p-1">
                      {allTags.filter((t: DbTag) => !selectedTagIds.includes(t.cod)).map((tag: DbTag) => (
                        <button
                          key={tag.cod}
                          onClick={() => handleAddSelectedTag(tag.cod)}
                          className={`px - 3 py - 1.5 bg - [var(--color - background)]hover: bg - [var(--color - border)]text - [var(--color - text - primary)]rounded - md transition - colors text - sm border border - [var(--color - border)] ${tag.restriccion_tag ? 'border-red-200' : ''} `}
                        >
                          + {tag.nombre_tag}
                        </button>
                      ))}
            ...
            <div className="px-6 py-6 space-y-4">
              <div>
                ...
                >
                  ...
                  {availableHotels.map((h: HotelItem) => (
      ...
      {isTagModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-[var(--color-card)] rounded-lg shadow-xl max-w-md w-full">
            <div className="px-6 py-4 border-b border-[var(--color-border)]">
              <h2 className="text-[var(--color-text-primary)] font-bold">Create New Tag</h2>
            </div>

            <div className="p-6 space-y-4">
              <div>
                <label className="block text-sm text-[var(--color-text-primary)] mb-1">Tag Name</label>
                <input
                  type="text"
                  value={newTagData.nombre_tag}
                  onChange={(e) => setNewTagData({ ...newTagData, nombre_tag: e.target.value })}
                  className="w-full px-4 py-2 bg-[var(--color-input-bg)] border border-[var(--color-border)] rounded-md focus:outline-none focus:ring-2 focus:ring-[var(--color-primary-blue)]"
              ...
              <div className="flex items-center gap-2">
                <input
                  type="checkbox"
                  id="is-restriction"
                  checked={newTagData.restriccion_tag}
                  onChange={(e) => setNewTagData({ ...newTagData, restriccion_tag: e.target.checked })}
                  className="w-4 h-4 text-[var(--color-primary-blue)]"
                />
                <label htmlFor="is-restriction" className="text-sm text-[var(--color-text-primary)]">Is this a restriction?</label>
              </div>

              {newTagData.restriccion_tag && (
                <div className="space-y-4 p-4 bg-[var(--color-background)] rounded-md border border-[var(--color-border)]">
                  <p className="text-xs font-bold text-[var(--color-text-secondary)] uppercase">Restriction Conditions</p>

                  <div>
                    <label className="block text-xs text-[var(--color-text-secondary)] mb-1">User Attribute</label>
                    <select
                      value={newTagData.condicion1_tag}
                      onChange={(e) => setNewTagData({ ...newTagData, condicion1_tag: e.target.value })}
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      ...
                      <select
                        value={newTagData.condicional_tag}
                        onChange={(e) => setNewTagData({ ...newTagData, condicional_tag: e.target.value })}
                        className="w-full px-3 py-1.5 text-sm bg-[var(--color-card)] border border-[var(--color-border)] rounded-md"
                      >
                        <option value="=">=</option>
                        <option value=">">&gt;</option>
                    ...
                    <div>
                      ...
                      <input
                        type="text"
                        value={newTagData.condicion2_tag}
                        onChange={(e) => setNewTagData({ ...newTagData, condicion2_tag: e.target.value })}
                        className="w-full px-3 py-1.5 text-sm bg-[var(--color-card)] border border-[var(--color-border)] rounded-md"
            ...
            <div className="px-6 py-4 border-t border-[var(--color-border)] flex justify-end gap-3">
              <button
                ...
                className="px-4 py-2 text-sm text-[var(--color-text-secondary)] hover:bg-gray-100 rounded-md transition-colors"
              >
                Cancel
              </button>
              <button
                onClick={handleCreateTag}
                className="px-4 py-2 text-sm bg-[var(--color-primary-blue)] text-white rounded-md hover:bg-[var(--color-primary-blue-hover)] transition-colors"
              >
                Create Tag
              </button>
            </div>
